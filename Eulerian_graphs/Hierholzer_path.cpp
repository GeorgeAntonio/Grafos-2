...
public:
    ...
    // Verificar se o grafo é Semieuleriano (Caminho Euleriano)
    bool verificarSemieuleriano() {
    ...
    // Encontrar um vértice de grau ímpar
    int encontrarVerticeImpar() {
        for (const auto &par : adj) {
            if (par.second.size() % 2 != 0) {
                return par.first;
            }
        }
        return 0; // Retorna 0 se não houver vértice ímpar (válido para ciclos Eulerianos)
    }

    // Algoritmo de Hierholzer para encontrar o caminho Euleriano
    vector<int> encontrarCaminhoEuleriano() {
        vector<int> caminho;
        if (!verificarSemieuleriano()) {
            cout << "O grafo não possui um Caminho Euleriano!" << endl;
            return caminho;
        }

        int inicio = encontrarVerticeImpar();
        stack<int> pilha;
        pilha.push(inicio);
...
int main() {
    ...
    vector<int> caminhoEuleriano = g.encontrarCaminhoEuleriano();

    if (!caminhoEuleriano.empty()) {
        cout << "Caminho Euleriano encontrado:\n";
        for (int vertice : caminhoEuleriano) {
            cout << vertice + 1 << " ";
        }
        cout << endl;
    }
